 ...
 */
/*
 * Source: http://developer.android.com/training/multiscreen/index.html
 */
public class NonsenseGenerator {
    ...
    public NonsenseGenerator(boolean html) {
    	this.html = html;
    ...
    public String makeHtmlParagraphs(int minParagraphs, int maxParagraphs, int minSentences, int maxSentences) {
        StringBuilder sb = new StringBuilder();
    	int p = minParagraphs+(maxParagraphs>minParagraphs?mRandom.nextInt(maxParagraphs-minParagraphs):0);
    	for(int i=0; i<p; i++) {
        	int s = minSentences+(maxSentences>minSentences?mRandom.nextInt(maxSentences-minSentences):0);
        	if(html) {
        		sb.append("<p>").append(makeText(s)).append("</p>");
        	} else {
        		sb.append(makeText(s));
        	}
    	}
        return sb.toString();
    }
